...
export default async function fetchDiscordMember(accessToken: AccessToken): Promise<DiscordMember> {
  try {
    const discordApi = getDiscordApiInstance(accessToken)
    const { data: member } = await discordApi.get(`/users/@me/guilds/${fwgGuildId}/member`)
    return member
  } catch (error) {
